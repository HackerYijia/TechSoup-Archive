...
# run it!!
cd bungee
tmux new -d -s server "java -Xmx2048M -jar bungee.jar; tmux kill-session -t server"
cd ..
cd server
tmux splitw -t server -v "java -Djline.terminal=jline.UnsupportedTerminal -Xmx2048M -jar server.jar nogui; tmux kill-session -t server"
